...
// GET /api/ec/candidates?page=1&limit=20&constituencyId=1
router.get("/candidates", async (req, res, next) => {
  try {
    ...
    const result = constituencyId
      ? await candidateService.getByConstituency(constituencyId, {
          page,
          limit,
        })
      : await candidateService.getAll({ page, limit });
