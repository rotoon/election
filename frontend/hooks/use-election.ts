...
export function useCandidates(constituencyId?: string | number | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.data || []).map((c: UseCandidatesApiItem) => ({
        ...c,
        id: Number(c.id),
...
export function useManageCandidates(constituencyId?: string | number | null) {
  return useQuery({
    queryKey: ["manage-candidates", constituencyId],
    queryFn: async () => {
      let url = "/ec/candidates?limit=1000";
      ...
      interface ApiManageCandidate {
        ...
        constituency?: { id: number; zoneNumber: number; isPollOpen: boolean };
      }

      return (data.data || []).map((c: ApiManageCandidate) => ({
        ...c,
        first_name: c.firstName,
        last_name: c.lastName,
        candidate_number: c.candidateNumber,
